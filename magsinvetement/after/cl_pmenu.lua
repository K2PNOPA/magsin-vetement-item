...
function PMenu:drawMenuButton(button, intX, intY, boolSelected, intW, intH, intID)
	...
	local Lock = (self.Events["setLock"] and self.Events["setLock"](button, self.Data.currentMenu, self)) or (button.Lock and button.Lock) 
	if Lock and string.len(Lock) > 0 then
	drawSprite("commonmenu", "shop_lock", intX + (intW / 2.15), intY, .02, .034, 0.0, tableColor[1], tableColor[2], tableColor[3], 255)
	end 

	local Triangle = (self.Events["setLock"] and self.Events["setLock"](button, self.Data.currentMenu, self)) or (button.Triangle and button.Triangle) 
	if Triangle and string.len(Triangle) > 0 then
	drawSprite("commonmenu", "mp_alerttriangle", intX + (intW / 3.7), intY, .02, .034, 0.0, 255, 255, 255, 255)
	end 

	local ABIGAIL = (self.Events["setLock"] and self.Events["setLock"](button, self.Data.currentMenu, self)) or (button.ABIGAIL and button.ABIGAIL) 
	if ABIGAIL and string.len(ABIGAIL) > 0 then
		if image ~= nil and image ~= "cca" then 

		drawSprite("personage", image, intX + (intW / 1.4) - .0000, intY - intH / 0.386, .04, .0663, 0.0, 255, 255, 255, 255)
		end   
	end 
...
function PMenu:DrawHeader(intCount)
	...
	if boolHeader then
		...
		if not self.Base.CustomHeader and headername ~= nil then
			...
			DrawText2(1, "vide", .7, self.Width  - spriteW / 2, self.Height - intH / 2 + .0125, color_white, false, 0)
		end
	end
...
function PMenu:DrawHelpers(tableButtons)
	...
	if hasHelp then
		...
		local nwintH = intH + (Linecount == 1 and 0 or ( (Linecount + 1) * 0.0075))
		self.Height = self.Height + intH / 2
		DrawSprite("commonmenu", "gradient_bgd", self.Width - _intW / 2, self.Height + nwintH / 2 - 0.0397, _intW, 0.035, .0, 255, 255, 255, 255)
		DrawSprite("commonmenu", "shop_arrows_upanddown", self.Width - _intW / 2, self.Height + nwintH / 2 - 0.0397, 0.03, 0.04, .0, 255, 255, 255, 255)
